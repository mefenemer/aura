...
window.initBilling = async function () {
    ...
    try {
        ...
        document.getElementById('billing-error-msg').textContent = e.message || 'Unable to load billing data.';
        _showState('error');
    }
};
...
function _renderSubscriptions(subs) {
    ...
    const active = subs.filter(s => ['active', 'trialing'].includes(s.stripeStatus || '') || s.status === 'active');
...
function _subscriptionCard(sub) {
    ...
    const pmLine = sub.paymentMethod
        ? `<div class="flex items-center gap-2 text-sm text-gray-600 mt-2">
             ...
             <span class="capitalize">${sub.paymentMethod.brand}</span>
             <span>ending in <strong>${sub.paymentMethod.last4}</strong></span>
      ...
      <div class="flex items-start justify-between gap-4 mb-4">
        <div>
          <div class="flex items-center gap-2 mb-1">
            <h3 class="text-base font-extrabold text-gray-900">${_escHtml(sub.planName)}</h3>
        ...
        <div class="text-right shrink-0">
          ...
          <p class="text-xs text-gray-400">per ${sub.billingCycle || 'month'}</p>
        </div>
      </div>
    </div>`;
}
...
function _renderHistory(payments) {
    ...
    tbody.innerHTML = payments.map(p => {
        ...
        const amount = p.amount
            ? `${p.currency === 'GBP' || p.currency === 'gbp' ? '£' : p.currency + ' '}${parseFloat(p.amount).toFixed(2)}`
            : '—';

        let pmCell = '—';
        if (p.paymentMethod && typeof p.paymentMethod === 'object') {
            pmCell = `<div class="flex items-center gap-1.5">${_cardIcon(p.paymentMethod.brand)}<span class="capitalize">${p.paymentMethod.brand}</span> ···· ${p.paymentMethod.last4}</div>`;
        } else if (typeof p.paymentMethod === 'string' && p.paymentMethod) {
            pmCell = p.paymentMethod;
        }

        const receiptCell = p.receiptUrl
            ? `<a href="${p.receiptUrl}" target="_blank" rel="noopener"
                class="text-emerald-600 hover:text-emerald-700 font-bold transition">
        ...
        <tr class="hover:bg-gray-50 transition">
          ...
          <td class="px-5 py-4 font-medium text-gray-900">${_escHtml(p.description || '—')}</td>
...
function _cardIcon(brand) {
    // Simple text-based card brand indicator
    const colors = {
        visa: 'text-blue-700', mastercard: 'text-red-600', amex: 'text-blue-500',
        discover: 'text-orange-600', default: 'text-gray-600',
    };
    const cls = colors[(brand || '').toLowerCase()] || colors.default;
